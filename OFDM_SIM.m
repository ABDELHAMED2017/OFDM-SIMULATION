...
% It requires a 256-grayscale bitmap file (*.png image file) as data source
...
clear;      % clear all previous data in Matlab workspace
clc;

SNR_dB_list = 0:2:20;
clipping_list = 0:2:10;
symbol_size = [1,2,4,8];

T_list = zeros(length(symbol_size), length(SNR_dB_list)*length(clipping_list));
Time_list = zeros(length(symbol_size), length(SNR_dB_list)*length(clipping_list));

for symol_loop = 1:length(symbol_size),
    for clipping_loop = 1:length(clipping_list),
        for snr_loop = 1:length(SNR_dB_list),
            
            fprintf('\n\n##########################################\n')
            fprintf('#*********** OFDM Simulation ************#\n')
            fprintf('##########################################\n\n')
            fprintf('# Symbol Size %d, SNR %d, clipping %d ********#\n\n', symol_loop, snr_loop, clipping_loop)
            
            % invoking ofdm_parameters.m script to set OFDM system parameters
            ofdm_parameters(symbol_size(symol_loop),clipping_list(clipping_loop),SNR_dB_list(snr_loop));
            % load saved parameters for receiver
            ...
            frame_num = 0;
            total_frame = ceil(length(baseband_tx)/(symb_per_frame*carrier_count));
            fig = 1;
            ...
            % pause;
            clear;      % flush all data stored in memory previously
            tic;    % start stopwatch
            ...
            last_frame = 0;
            unpad = 0;
            if rem(w*h*t, carrier_count)~=0
                unpad = carrier_count - rem(w*h*t, carrier_count);
            end
            num_frame = ceil((h*w*t)*(word_size/symb_size)/(symb_per_frame*carrier_count));
            fig = 0;
            for k = 1:num_frame
                ...
                if k==num_frame
                    ...
                    frame_end = min(end_x, (frame_start-1) + symb_period*...
                        (1+ceil(rem(w*h*t, carrier_count*symb_per_frame)/carrier_count)));
                else
            ...
            T_list(symol_loop,(clipping_loop -1)*length(SNR_dB_list) + snr_loop) = toc;
            ...
            % patch or trim the data to fit a w-by-h-by-t image
            if length(data_out)>(w*h*t)        % trim extra data
                data_out = data_out(1:(w*h*t));
            elseif length(data_out)<(w*h*t)    % patch a partially missing row
                buff_h = h;
                h = ceil(length(data_out)/(w*t));
                ...
                data_temp = zeros(1,w*h*t);
                data_temp(1:length(data_out)) = data_out;
                if length(data_out)~=(w*h*t)
                    for k = 1:(w*h*t-length(data_out))
                        pad_pixel = data_temp(length(data_out) - w + k);
                        data_temp(length(data_out) + k) = pad_pixel;
                    end
                end
                data_out = data_temp;
            end
            ...
            data_out = reshape(data_out, h, w,[]);
            data_out = uint8(data_out);
            % save the output image to a bitmap (*.png) file
            imwrite(data_out, file_out, 'png');
            ...
            bit_error =  length(errors)/length(data_rx)*100;
            % Bit Error Rate
            fprintf('Bit Error Rate (BER) = %f%%\n',bit_error)
            T_list(symol_loop,(clipping_loop -1)*length(SNR_dB_list) + snr_loop) = bit_error;
            ...
            fprintf('##########################################\n\n')
            [img_rece,map] = imread('image_OFDM.png');
            [img_sent,map0] = imread('image.png');
            
            figure(8)
            subplot(121)
            imshow(img_sent,map0)
            title(['Image Sent, Clip = ', num2str(clipping),'dB, Symb Size = ', num2str(symb_size)])
            subplot(122)
            imshow(img_rece,map)
            title(['Image Received, SNR = ', num2str(SNR_dB),'dB'])
        end
    end
end